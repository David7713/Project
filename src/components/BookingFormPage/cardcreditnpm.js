...
const BookingFormPage = () => {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    if (!values.name || !values.surname || !values.email || !values.phone || !values.city || !values.street || !values.zip || !values.cardHolderName || !cardNumber || !expiry || !cvc) {
  ...
  const validateExpiry = (value) => {
    // Implement your card expiry validation logic here
    // For simplicity, let's say the expiry date must be in MM/YY format
    const regex = /^(0[1-9]|1[0-2])\/\d{2}$/;
    return regex.test(value);
  };
  
  const validateCVC = (value) => {
    // Implement your card CVC validation logic here
    // For simplicity, let's say the CVC must be a 3-digit number
    const regex = /^[0-9]{3}$/;
    return regex.test(value);
  };
  const openModal = () => {
  ...
  return (
    <div className='booking-form-section'>
      ...
      />
         ...
         <div className='credit-card-input-section'>
          <CreditCardInput
